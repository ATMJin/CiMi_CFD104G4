...
if(!empty($mem_id) && !empty($mem_psw) && !empty($mem_name) && !empty($mem_birth) && !empty($mem_mail) && !empty($board1) && !empty($board2) && !empty($board2)){
    ...
    // }else{
        ...
        if(filter_var($mem_mail, FILTER_VALIDATE_EMAIL)){
            ...
            if($email -> rowCount() > 0){
                ...
                //sql for mem 表格
                $sql2 = "INSERT INTO mem (mem_no, mem_id, mem_psw, mem_name, mem_head, mem_mail) 
                ...
                $sql3 = "INSERT INTO mempairdata (mem_no, mem_birth) 
